    ...
    def test_scraper_format_company_info_never_shows_nan(self):
        ...
        for invalid_value in self.invalid_values:
            test_rows = [
                ...
                {
                    "company_industry": invalid_value,
                    "company_num_employees": invalid_value,
                    "company_revenue": invalid_value,
                },
                {
        ...
        for column in text_columns:
            if column in processed_jobs.columns:
                for value in processed_jobs[column].fillna(""):
                    self.assertNotIn("nan", str(value).lower(), f"Found 'nan' in {column}: {value}")
        ...
        for i, scenario in enumerate(realistic_scenarios):
            with self.subTest(scenario=i):
                ...
                for field, cleaned_value in cleaned_data.items():
                    self.assertNotIn(
                        "nan", str(cleaned_value).lower(), f"Found 'nan' in cleaned {field}: {cleaned_value}"
                    )
                    self.assertNotIn(
                        "none", str(cleaned_value).lower(), f"Found 'none' in cleaned {field}: {cleaned_value}"
                    )
                    self.assertNotIn(
                        "null", str(cleaned_value).lower(), f"Found 'null' in cleaned {field}: {cleaned_value}"
                    )
