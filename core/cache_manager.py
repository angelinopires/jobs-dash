...
class CacheManager:
    ...
    def cache_result(self, cache_key: str, result: Dict[str, Any]) -> None:
        ...
        try:
            session_entry = {
                "timestamp": cache_entry["timestamp"],
                "result": result  # Original result with DataFrame
            }
        ...
        try:
            ...
            if os.path.exists(self.cache_file):
                try:
                    ...
                    print(f"Warning: Cache file corrupted, creating new cache")
                    file_cache = {}
    ...
    def _cleanup_file_cache(self) -> None:
        ...
        try:
            ...
            valid_cache = {
                key: entry for key, entry in file_cache.items() 
            ...
            print(f"Warning: Cache file corrupted during cleanup, clearing cache")
            self._clear_corrupted_cache()
        except Exception as e:
    ...
    def clear_scraper_cache(self, scraper_name: str) -> None:
        ...
        try:
            keys_to_remove = [
                key for key in st.session_state.job_search_cache.keys() 
                if key.startswith(f"{scraper_name}_")
            ]
    ...
    def _clear_scraper_file_cache(self, scraper_name: str) -> None:
        ...
        try:
            ...
            cleaned_cache = {
                key: entry for key, entry in file_cache.items() 
                if not key.startswith(f"{scraper_name}_")
            }
